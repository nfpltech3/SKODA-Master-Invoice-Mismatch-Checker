...
3. **Load Extracted Invoice**: Click the **📂 Extracted Invoice** button and select the invoice extraction file.
   - *Constraint: The file must have a sheet containing core fields such as `Invoice Number`, `Invoice Date`, `Mat. NO.`, `Country of Origin`, `Quantity`, `Total Price`, and `Description`. (The `Price per PC` column is optional and will be matched against Unit if present). The tool automatically handles alternative column headers and Bentley CSV formats (e.g., `Part Number`, `COO`, `Qty`, `HS-CODE`, `VALUE OF GOODS`, `INVOICE NUMBER`, `INVOICE DATE`, `PART NUMBER`, `EXTENDED PRICE`, `PART DESCRIPTION`, `HARMONISED COMMODITY CODE`).*
   - *Note: **This file is now optional.** You can run a partial reconciliation using just the Master Sheet and Item Report. The tool will prompt you if you proceed without the invoice.*
4. **Execute Reconciliation**: Click the bright red **▶ Run** button (it unlocks as soon as the Master Sheet and Item Report are loaded). The button will shift to a dark red **⌛ Processing...** state while compiling your audit.
